...
\begin{tikzpicture}[node distance=1.0cm, >=Latex]
  \node (L0) [draw, rounded corners] {$L_0$};
  \node (R0) [draw, rounded corners, right=2.5cm of L0] {$R_0$};
  \node (f1) [draw, rounded corners, below=1.2cm of R0, minimum width=1.8cm] {$F(\cdot, K_1)$};
  \node (xor1) [below=1.2cm of L0] {$\oplus$};
  \draw[->] (R0) -- (f1);
  \draw[->] (f1) -- (xor1);
  \draw (L0) -- (xor1);
  \node (L1) [draw, rounded corners, below=2.2cm of xor1] {$L_1=R_0$};
  \node (R1) [draw, rounded corners, below=2.2cm of f1] {$R_1=L_0\oplus F(R_0,K_1)$};
\end{tikzpicture}
